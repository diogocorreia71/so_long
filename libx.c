...
/*   Updated: 2023/09/22 10:17:25 by diodos-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_sprites(t_game *so_long)
{
	...
	so_long->sprites[S1].img = mlx_xpm_file_to_image(so_long->mlx, FS1, &(so_long->sprites[S1].width), &(so_long->sprites[S1].height));
...
void	render_position(t_game *so_long, int x, int y)
{
	if (so_long->map[y][x] == WALL)
		mlx_put_image_to_window(so_long->mlx, so_long->win, so_long->sprites[W1].img, so_long->sprites[W1].width * x, so_long->sprites[W1].height * y);
	else if (so_long->map[y][x] == COIN)
		mlx_put_image_to_window(so_long->mlx, so_long->win, so_long->sprites[C1].img, so_long->sprites[C1].width * x, so_long->sprites[C1].height * y);
	else if (so_long->map[y][x] == EXIT)
		mlx_put_image_to_window(so_long->mlx, so_long->win, so_long->sprites[E1].img, so_long->sprites[E1].width * x, so_long->sprites[E1].height * y);
	else if (so_long->map[y][x] == SPACE)
		mlx_put_image_to_window(so_long->mlx, so_long->win, so_long->sprites[S1].img, so_long->sprites[S1].width * x, so_long->sprites[S1].height * y);
	else if (so_long->map [y][x] == PLAYER)
		mlx_put_image_to_window(so_long->mlx, so_long->win, so_long->sprites[P1].img, so_long->sprites[P1].width * x, so_long->sprites[P1].height * y);
}